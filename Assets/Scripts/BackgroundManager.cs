...
public class BackgroundManager : MonoBehaviour
{
    ...
    void generate()
    {
        ...
        Transform houseLeft = Instantiate(housePrefabs[hi], new Vector3(player.transform.position.x - 7.0f, player.transform.position.y - 2.0f, largestZ), Quaternion.identity);
        hi = Random.Range(0, housePrefabs.Length);
        Transform houseRight = Instantiate(housePrefabs[hi], new Vector3(player.transform.position.x + 7.0f, player.transform.position.y - 2.0f, largestZ), Quaternion.AngleAxis(180.0f, Vector3.up));
